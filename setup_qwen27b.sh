...
# -----------------------------------------------------------------------
# Beam Node Agent - Setup Script for Qwen 3.5 27B (Ollama single-node mode)
# Usage: bash setup_qwen27b.sh
# Requires: ~14 GB VRAM (Q4), 16-24 GB GPU recommended
# -----------------------------------------------------------------------

CONTROL_PLANE_URL="${BEAM_CONTROL_PLANE_URL:-https://www.openbeam.me}"
OLLAMA_MODEL="${BEAM_OLLAMA_MODEL:-qwen3.5:27b}"
...
echo "=== Beam Node Agent Setup (Qwen 3.5 27B) ==="
echo "Control plane: $CONTROL_PLANE_URL"
echo "Model: $OLLAMA_MODEL"
echo "GPU: $GPU_NAME (${GPU_VRAM}GB x${GPU_COUNT})"
echo ""

# Check VRAM is sufficient (~14 GB minimum for Q4)
if [ "$GPU_VRAM" -gt 0 ] && [ "$GPU_VRAM" -lt 14 ]; then
    echo "WARNING: Qwen 3.5 27B requires ~14 GB VRAM (Q4 quantization)."
    echo "         Detected ${GPU_VRAM} GB. The model may not fit."
    echo "         Consider using setup_runpod.sh (Qwen 3.5 35B-A3B, ~16 GB)"
    echo "         or setup_mimo.sh (MiMo-7B, ~5 GB) instead."
    echo ""
...
agent:
  ...
  capabilities:
    ...
    max_model_class: "C"
    preferred_model_id: "Qwen/Qwen3.5-27B-Ollama"
EOF
